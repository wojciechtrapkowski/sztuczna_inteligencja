...
class QAgent(Agent):
    ...
    def __init__(
        self, n_states, n_actions, name="QAgent", initial_q_value=1000.0, q_table=None
    ):
        super().__init__(name)
        ...
        self.eps_decrement = (
            0.01  # wartość, o którą zmniejsza się epsilon po każdym kroku
        )
        self.eps_min = (
            0.01  # końcowa wartość epsilon, poniżej którego już nie jest zmniejszane
        )

        self.action_space = [i for i in range(n_actions)]
        self.n_states = n_states
        self.q_table = (
            q_table if q_table is not None else self.init_q_table(initial_q_value)
        )

    def init_q_table(self, initial_q_value=0.0):
    ...
    def choose_action(self, state: State) -> Action:
        ...
        # na razie agent zawsze wybiera akcję "idź do góry"

    def learn(
        self, state: State, action: Action, reward: float, new_state: State, done: bool
    ) -> None:
