...
class FrozenLake(Env):
    def __init__(self, name="FrozenLake"):
        ...
        self.n_states = len(self.grid_world) * len(self.grid_world[0]) * 4
        ...
        assert (
            ...
            + 2 * self.side_from_intended_prob
            + self.opposite_from_intended_prob
            == 1.0
        ...
        self.cave_entry_x = 0
        self.cave_entry_y = (
            len(self.grid_world) - 1
        )  # the entry point is the bottom left cell
    ...
    def get_state(self, y, x) -> int:
        ...
        return int(
            int(self.n_states) / 2 * int(self.grid_world[-1][-1] == self.regular_field)
            + int(self.n_states / 4) * int(self.grid_world[0][-1] == self.regular_field)
            + len(self.grid_world) * y
            + x
        )
    ...
    def step(self, action):
        ...
        if action == intended_action:
            ...
            info.append(
                f"Agent wanted {self.action_names[intended_action]}, but moved {self.action_names[action]}."
            )
        ...
        if (
            self.grid_world[self.agentPosYX[0]][self.agentPosYX[1]]
            == self.regular_field
        ):
            pass
            ...
            for x in range(len(self.grid_world[y])):
                if self.grid_world[y][x] == self.regular_field:
                    pygame.draw.rect(
                        screen,
                        BACKGROUND,
                        pygame.Rect(x * x_size, y * y_size, x_size, y_size),
                    )
                elif self.grid_world[y][x] == self.gold_field:
                    screen.blit(
                        self.assets["gold"], (x * x_size, y * y_size, x_size, y_size)
                    )
                    ...
                    screen.blit(
                        self.assets["coin"], (x * x_size, y * y_size, x_size, y_size)
                    )
                elif self.grid_world[y][x] == self.pit_field:
                    screen.blit(
                        self.assets["pit"], (x * x_size, y * y_size, x_size, y_size)
                    )

                best = None
                if q_values is not None and (
                    self.grid_world[y][x] == self.regular_field
                ):

                    state = self.get_state(y, x)
                    sorted_actions = np.argsort(q_values[state])
                    if (
                        q_values[state, sorted_actions[3]]
                        > q_values[state, sorted_actions[2]]
                    ):  # there is a clear best action
                        best = sorted_actions[3]

                    arrows = {
                        0: self.assets["arrow_up"],
                ...
                if (y == self.agentPosYX[0]) & (x == self.agentPosYX[1]):
                    screen.blit(
                        self.assets["agent"], (x * x_size, y * y_size, x_size, y_size)
                    )

                pygame.draw.rect(
                    screen, BLUE, pygame.Rect(x * x_size, y * y_size, x_size, y_size), 5
                )
